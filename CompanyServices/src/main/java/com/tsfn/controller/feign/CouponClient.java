...
@FeignClient(name = "coupon-service", url = "http://localhost:8080") // Specify the name and URL of the coupon service
public interface CouponClient {
    ...
    ResponseEntity<?> deleteCoupon(@PathVariable("couponId") int couponId);

    @GetMapping("/coupons/company/{companyId}")
    ResponseEntity<?> getAllCompanyCoupons(@PathVariable("companyId") int companyId);

    @GetMapping("/coupons/company/{companyId}/category/{   }")
    ResponseEntity<?> getCompanyCouponsByCategory(@PathVariable("companyId") int companyId, @PathVariable("category") Category category);
